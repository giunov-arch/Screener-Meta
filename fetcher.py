...
ROOT = QUI.parent if (QUI / ".." / "dati").exists() or (QUI.name == "github-screener-italia") else QUI
# Prova a trovare root del repo (dove c'è fetch.yml)
for p in [QUI, QUI.parent, Path.cwd()]:
    if (p / "fetch.yml").exists() or (p / ".github").exists():
        ROOT = p
        break
UNIVERSO_PATH = QUI / "universo.json"  # se esiste, usa static fallback come collector
OUTPUT_SNAPSHOT = QUI / "dati" / "snapshot.json"
OUTPUT_SNAPSHOT_ROOT = ROOT / "dati" / "snapshot.json"
OUTPUT_SNAPSHOT_DATA = ROOT / "data" / "snapshot.json"
OUTPUT_FLAT_JSON = QUI / "data" / "italian_stocks.json"
OUTPUT_FLAT_JSON_ROOT = ROOT / "data" / "italian_stocks.json"
OUTPUT_FLAT_CSV = QUI / "data" / "italian_stocks.csv"
OUTPUT_FLAT_CSV_ROOT = ROOT / "data" / "italian_stocks.csv"
